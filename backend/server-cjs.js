...
const initSampleData = () => {
  ...
  if (productCount === 0) {
    ...
    const products = [
      { name: '魔芋精粉 K-100', category: '魔芋精粉', description: '采用优质魔芋精制而成，粘度高，纯度达95%以上，广泛应用于食品工业', specification: '纯度≥95% | 粘度≥25000mpa·s | 细度200目', price: 28.00, image: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400', featured: 1 },
      { name: '魔芋微粉 K-200', category: '魔芋微粉', description: '超微研磨技术，颗粒更细，溶解性更强，适合高端食品应用', specification: '纯度≥90% | 粘度≥30000mpa·s | 细度300目', price: 35.00, image: 'https://images.unsplash.com/photo-1586201375761-83865001e31c?w=400', featured: 1 },
      { name: '魔芋膳食纤维', category: '膳食纤维', description: '高纯度魔芋膳食纤维，健康食品首选，零脂肪低热量', specification: '膳食纤维≥90% | 胶质≥85%', price: 42.00, image: 'https://images.unsplash.com/photo-1490474418585-ba9bad8fd0ea?w=400', featured: 0 },
      { name: '魔芋凝胶粉', category: '功能性配料', description: '特制凝胶配方，成型效果好，适用于果冻、布丁等制品', specification: '凝胶强度≥200g/cm² | 凝固时间5-10分钟', price: 32.00, image: 'https://images.unsplash.com/photo-1558326567-98ae2405596b?w=400', featured: 1 },
      { name: '魔芋复配粉', category: '复配产品', description: '科学配比多种功能性成分，满足多样化应用需求', specification: '魔芋粉+果胶+卡拉胶 | 定制化配比', price: 38.00, image: 'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=400', featured: 0 },
      { name: '魔芋素食品专用粉', category: '魔芋精粉', description: '专为素食食品开发，口感接近肉类，富含植物蛋白', specification: '纯度≥92% | 蛋白质≥8% | 粘度≥28000mpa·s', price: 30.00, image: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=400', featured: 0 }
    ];

    const insertProduct = db.prepare(`INSERT INTO products (name, category, description, specification, price, image, featured) VALUES (?, ?, ?, ?, ?, ?, ?)`);
    products.forEach(p => insertProduct.run(p.name, p.category, p.description, p.specification, p.price, p.image, p.featured));
  }
  ...
  if (newsCount === 0) {
    ...
    news.forEach(n => insertNews.run(n.title, n.summary, n.content, n.image, n.published));
  }
...
app.post('/api/products', (req, res) => {
  try {
    ...
    const result = db.prepare(`INSERT INTO products (name, category, description, specification, price, image, featured) VALUES (?, ?, ?, ?, ?, ?, ?)`)
...
app.put('/api/products/:id', (req, res) => {
  try {
    const { name, category, description, specification, price, image, featured } = req.body;
    db.prepare(`UPDATE products SET name = ?, category = ?, description = ?, specification = ?, price = ?, image = ?, featured = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
